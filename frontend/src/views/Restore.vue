    ...
    <v-card max-width="75%" class="mx-auto mt-4">
      ...
      <v-card-text>
        <v-form v-model="valid" ref="form">
          <v-file-input chips :rules="rules" label="Backup JSON" v-model="file" required></v-file-input>
        </v-form>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn class="mr-6 mb-2" @click="restore" :disabled="!valid">Restore</v-btn>
      </v-card-actions>
    </v-card>
  ...
  methods: {
    ...
    async restoreMod(mod) {
      ...
      if (response.status !== 200) {
        throw `Could not add mod '${mod.name}'`;
      }

      const promises = [];

      for (let update of mod.updates) {
        promises.push(
          axios.post(
